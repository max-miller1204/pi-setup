...
available-skills: iterative-review, reviewed-pr, playwright-cli
...
When the skill requires a full review pass, spawn agent `review-pass` in the repository root. Set `profile: selectedProfile` after selecting an approved model and thinking choice from the active `subagent` tool description. Choose a capable profile for a full review. A project policy can replace the global list, so check the active choices before each spawn. Use a new session and a unique name for every pass, including retries, such as `review-pass-round-0`, `review-pass-round-1`, and so on. Never resume a previous pass to certify fixes.
...
The `review-pass` agent is intentionally hidden from `subagents_list`. Its absence from that list is expected. Spawn it directly with `agent: "review-pass"` and an approved `profile`. Do not ask the user to expose it. If it cannot start, report `blocked`; do not replace it with your own review or another helper.
